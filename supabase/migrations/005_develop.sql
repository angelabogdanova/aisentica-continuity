...
begin
  ...
  if v_agent.status <> 'ACTIVE' then
    raise exception 'Only ACTIVE agents can Develop.' using errcode = 'P0001';
  end if;

  if v_agent.canonical_domain is null or v_agent.current_version < 2 then
    raise exception 'Verified canonical domain required.' using errcode = 'P0001';
  end if;
...
revoke execute on function public.develop_agent(text, text, jsonb)
  from public, anon, authenticated;

grant execute on function public.develop_agent(text, text, jsonb)
  to service_role;

...
const ai: AIService = { generateManifest: vi.fn().mockResolvedValue(manifest) };

describe('Phase 1 lifecycle', () => {
  beforeEach(() => { process.env.NODE_ENV = 'test'; process.env.STORAGE_BACKEND = 'memory'; delete process.env.DEMO_SESSION_SECRET; });
  ...
  it('validates before invoking AI', async () => { const service = { generateManifest: vi.fn() }; await expect(createAgentLifecycle({ ...values, purpose: 'short' }, 'owner-a', { ai: service, repository: new MemoryRepository() })).rejects.toThrow(); expect(service.generateManifest).not.toHaveBeenCalled(); });
  it('does not persist after AI failure', async () => { const repo = new MemoryRepository(); await expect(createAgentLifecycle(values, 'owner-a', { ai: { generateManifest: async () => { throw new AIServiceError('Nope', 'api'); } }, repository: repo })).rejects.toThrow('Nope'); expect(await repo.byOwner('owner-a')).toHaveLength(0); });
  ...
  it('does not expose owner or private manifest data publicly', () => { const detail = { agent: { id: 'AC-TEST123', canonicalName: 'Atlas', role: 'Research Agent', purpose: manifest.purpose, field: 'Research', ownerId: 'owner-a', status: 'ACTIVE' as const, canonicalDomain: null, currentVersion: 1, createdAt: '', updatedAt: '' }, owner: { id: 'owner-a', displayName: 'Owner A', slug: 'owner-a', createdAt: '' }, version: { id: 'v', agentId: 'AC-TEST123', versionNumber: 1, versionType: 'INITIAL_MANIFEST' as const, stateJson: manifest, changeSummary: '', createdByOwnerId: 'owner-a', createdAt: '' }, events: [] }; const safe = publicAgent(detail); expect(safe).not.toHaveProperty('ownerId'); expect(safe).not.toHaveProperty('privateOwnerDataRules'); });
  it('selects only explicit storage and requires production secrets', () => { expect(storageBackend()).toBe('memory'); process.env.NODE_ENV = 'production'; delete process.env.DEMO_SESSION_SECRET; expect(() => demoSessionSecret()).toThrow('DEMO_SESSION_SECRET'); });

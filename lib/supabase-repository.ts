...
function mapAgent(row: Row): Agent {
  return { id: row.id as string, canonicalName: row.canonical_name as string, role: row.role as string, purpose: row.purpose as string, field: row.field as string, ownerId: row.owner_id as string, status: row.status as Agent['status'], canonicalDomain: row.canonical_domain as string | null, currentVersion: row.current_version as number, createdAt: row.created_at as string, updatedAt: row.updated_at as string };
}

function mapVersion(row: Row): Version {
  return { id: row.id as string, agentId: row.agent_id as string, versionNumber: row.version_number as number, versionType: row.version_type as Version['versionType'], stateJson: row.state_json as VersionState, changeSummary: row.change_summary as string, createdByOwnerId: row.created_by_owner_id as string, createdAt: row.created_at as string };
}
...
export function mapCreateRpcResult(value: unknown): AgentDetail {
  ...
  return { agent: mapAgent(asRow(row.agent, 'Agent')), owner: mapOwner(asRow(row.owner, 'Owner')), version, versions: [version], events: [mapEvent(asRow(row.event, 'Agent Event'))] };
}
...
export class SupabaseRepository implements Repository {
  ...
  async detail(id: string): Promise<AgentDetail | undefined> {
    ...
    return { agent: mapAgent(record), owner: mapOwner(asRow(record.demo_owners, 'Demo Owner')), version: mapVersion(version), versions, events: asRows(record.agent_events, 'Agent Event').map(mapEvent).sort((a, b) => a.createdAt.localeCompare(b.createdAt)) };
  }

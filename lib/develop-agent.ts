...
export async function developAgentLifecycle(values: Record<string, unknown>, agentId: string, ownerId: string, dependencies: { repository: Repository; development: DevelopmentService }): Promise<string> {
  ...
  if (!detail.agent.canonicalDomain || detail.agent.currentVersion < 2) throw new Error('A verified canonical domain is required before Develop.');

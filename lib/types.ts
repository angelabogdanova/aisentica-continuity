...
export type VersionState = Manifest & { canonicalDomain?: string; domainVerificationStatus?: 'VERIFIED'; domainVerifiedAt?: string; developmentHistory?: DevelopmentRecord[]; latestDevelopment?: DevelopmentRecord; parkHistory?: ParkRecord[]; latestPark?: ParkRecord };
...
export type Version = { id: string; agentId: string; versionNumber: number; versionType: 'INITIAL_MANIFEST' | 'DOMAIN_BINDING' | 'DEVELOPMENT' | 'PARKED'; stateJson: VersionState; changeSummary: string; createdByOwnerId: string; createdAt: string };
export type Event = { id: string; agentId: string; eventType: 'CREATE' | 'BIND_DOMAIN' | 'DEVELOP' | 'PARK'; actorOwnerId: string; metadataJson: Record<string, string>; createdAt: string };

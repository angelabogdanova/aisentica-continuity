...
export function normalizeRequestHost(value: string | null): string {
  ...
  if (/^\d{1,3}(?:\.\d{1,3}){3}$/.test(host) || host.includes('[') || host.includes(']')) throw new Error('IP address hosts cannot be canonical domains.');
...
export class SameOriginHttpsDomainVerificationService implements DomainVerificationService {
  ...
  async verify(expected: DomainProof): Promise<void> {
    const response = await this.fetcher(`https://${expected.domain}/.well-known/aisentica-continuity/${encodeURIComponent(expected.agentId)}`, { cache: 'no-store', redirect: 'error' });

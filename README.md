...
Create a Supabase project and apply migrations `001_phase1.sql` through `005_develop.sql` in filename order (or run `supabase db push`). Migration 003 aligns the Create RPC and production access controls; migration 004 adds owner-authorized domain-binding RPCs, binding constraints, and verified-domain uniqueness without deleting existing lifecycle data. The separate `supabase/seed.sql` remains available for local resets.
...
This phase intentionally does not accept arbitrary URLs: constraining verification to the current request host avoids SSRF and creates a deterministic demonstration. External customer domains can later use DNS TXT or an externally hosted well-known proof. Migration `004_bind_domain.sql` preserves all existing Agents and lifecycle history.

## Phase 3 — Develop

Domain-bound ACTIVE Agents can accumulate durable professional state through bounded owner work episodes. The server validates Task, Context and Evidence, and Success Criteria before calling GPT-5.6. Owner-supplied context is treated as untrusted material—not automatically verified truth—and the structured-output prompt prohibits invented verification, secrets, personal identifiers, and irrelevant private data.

A successful operation atomically creates the next sequential `DEVELOPMENT` version and one `DEVELOP` event. The complete prior state is copied forward immutably, the new safe Development Record is appended to `developmentHistory`, and repeated Develop operations create Versions 4, 5, and later without replacing earlier records. Public cards expose only `publicDevelopmentSummary`; raw submissions and private structured state remain owner-only.

Apply Supabase migrations in order: `001 → 002 → 003 → 004 → 005`. Park, Reactivate, Transfer, and Continue remain unimplemented.

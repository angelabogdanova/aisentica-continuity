...
export function AgentDetailView({ detail, privateView, currentDomain }: { detail: AgentDetail; privateView: boolean; currentDomain?: string }) {
  ...
  return <main className="mx-auto max-w-5xl px-5 py-12">
    ...
    {privateView&&currentDomain&&<BindDomainForm agentId={agent.id} domain={currentDomain}/>} 
    ...
    <section className="card mt-5">
      ...
      {privateView
        ...
        : <div className="mt-3 space-y-3 text-zinc-300"><p>{version.stateJson.publicIdentitySummary}</p>{latest&&<><span className="inline-block border border-red-500 px-2 py-1 text-xs text-red-200">Developed professional state</span><p>{latest.publicDevelopmentSummary}</p></>}</div>}
    </section>

    {privateView&&<>
      <section className="card mt-5">
        <h2 className="text-xl font-bold">State timeline</h2>
        {events.map((event)=><div key={event.id} className="mt-4 border-l-2 border-red-500 pl-4"><b>v{event.metadataJson.version} · {event.eventType}</b><p className="text-sm text-zinc-400">{event.eventType==='CREATE'?'Initial Agent Manifest created.':event.eventType==='BIND_DOMAIN'?`Verified canonical domain ${event.metadataJson.domain}.`:'Developed durable professional state.'} {new Date(event.createdAt).toLocaleString()}</p></div>)}
      </section>
      <section className="card mt-5">

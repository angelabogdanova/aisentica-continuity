...
export function Lifecycle({ events = [] }: { events?: Event[] }) {
  ...
  const completed = [state.create, state.bindDomain, state.develop, state.park, false, false, false];
  ...
  return <ol className="grid grid-cols-2 gap-2 sm:grid-cols-7">{steps.map((step, index) => {
    ...
    const label = done ? 'Completed' : next && index <= 3 ? 'Available' : next ? 'Next phase' : 'Later phase';
